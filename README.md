...
`crosscheck onboard` walks you through six steps: environment check, deployment mode (personal vs team), repo selection, review mode (cross-vendor or single-vendor), workflow pipeline (review-only / review+fix / review+fix+recheck), and config write. `crosscheck watch` then opens a `localhost.run` SSH tunnel, auto-registers the webhook, and starts listening.
