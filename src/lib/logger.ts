...
export function classifyError(message: string): ErrorCategory {
  ...
  if (/bad credentials|401|not logged in|not authenticated|auth failure|github_token|authentication required|token/.test(m)) return 'auth'
  ...
  if (/fetch failed|econnrefused|enotfound|network error|socket hang|socket timeout|rpc failed|http\/2|http2|framing layer|early eof|invalid index-pack|unexpected disconnect|fetch-pack|recv failure|failed to connect|couldn't connect|connection reset|connection refused|curl (?:28|35|56|92)\b/.test(m)) return 'network'
